...
public class GameState : AState
{
    ...
    public Text coinTextPS;
    public Text CoinTextHighScore;
	...
	public Button mypauseButton;
    public GameObject m_pauseMenu;  // _PB - Added new
    ...
    public Text pauePanelScore;
    public GameObject quitePanel;
    public Animator timerpanel;
    public Text gameQuitSCore;
    ...
    private float leftCameraPosition = -5f;
    private float righgtCameraPosition = 5f;
    ...
    private void Start()
    {
        CoinTextHighScore.text = PlayerPrefs.GetInt("HighScore", 0).ToString();
      //  StartCoroutine(DisableSplashBlur());
    }

    //IEnumerator DisableSplashBlur()
 //   {
   ...
   // }
    ...
    private void Update()
    {
		if (Input.GetKey(KeyCode.Escape))
		{
			...
			{
                GameExit();
                gameQuitSCore.text = coinScore.ToString();
            }
		}
	}
    ...
    private void WholeUIPanel()
    {
        HelperUtil.CallAfterCondition(() =>
        {
            wholeUI.gameObject.SetActive(true);
        }, ()=>!GameState.instance.Splashblur.activeSelf);
    }
    ...
    public void StartGame()
    {
        ...
        mypauseButton.gameObject.SetActive(!trackManager.isTutorial);
	...
	void OnApplicationPause(bool pauseStatus)
	{
		...
		if (pauseStatus) GameExit();
        
	}

    void OnApplicationFocus(bool focusStatus)
    {
        if (!focusStatus) GamePause();
        if (!focusStatus) GameExit();
     
    }

    
    public void GamePause(bool displayMenu = true)
	{
        if (m_Finished || AudioListener.pause == true)
			return;
        EnemyCharacter.instance.PauseEnemyState();
        ...
        pauePanelScore.text = coinScore.ToString();
    }

    public void GameExit(bool displayMenu = true)
	{
        if (m_Finished || AudioListener.pause == true) return;

        quitePanel.gameObject.SetActive (displayMenu);
		AudioListener.pause = true;
		Time.timeScale = 0;
		pauseButton.gameObject.SetActive(false);
		mypauseButton.gameObject.SetActive(false);
    ...
    private async void TimeSDelay()
    {
        EnemyCharacter.instance.IsRun = false;
        isResume = true;
        EnemyCharacter.instance.m_enemyAnimator.enabled=false;
        CharacterInputController.instance.character.animator.enabled = false;
        Time.timeScale = 0f;
        pauseMenu.gameObject.SetActive(false);
        await System.Threading.Tasks.Task.Delay(3000);
        Time.timeScale = 1f;
        EnemyCharacter.instance.IsRun = !EnemyCharacter.instance.IsCatch;
        EnemyCharacter.instance.m_enemyAnimator.enabled = true;
        countdownTimer.SetActive(false);
        CharacterInputController.instance.character.animator.enabled = true;
    ...
    protected void UpdateUI()
    {
        ...
        coinTextPS.text = trackManager.characterController.coins.ToString();
        coinScore = trackManager.characterController.coins;
        gameOverScoreText.text = coinScore.ToString();

        if(coinScore > PlayerPrefs.GetInt("HighScore", 0))
        {
            PlayerPrefs.SetInt("HighScore", coinScore);
            CoinTextHighScore.text = coinScore.ToString();
        }
    ...
    public void OpenGameOverPopup()
    {
       ...
       // EnemyCharacter.instance.m_audio.Stop();
        ClearPowerup();
