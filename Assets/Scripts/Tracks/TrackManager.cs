...
public class TrackManager : MonoBehaviour
{
    ...
    protected float m_TimeToStart = 1f;
    ...
    public float k_CountdownToStartLength = 1f;
    protected const float k_CountdownSpeed = 5f;
    ...
    protected void Awake()
    {
        m_ScoreAccum = 0.0f;
        s_Instance = this;
        Debug.Log("Addressable Runitme Path: \n" + Addressables.RuntimePath);
    }
    ...
    IEnumerator WaitToStart()
    {
        ...
        //yield return null;
        float length = k_CountdownToStartLength;   //  k_CountdownToStartLength = 5f;
        m_TimeToStart = length;                    //  m_TimeToStart = -1.0f;

        while (m_TimeToStart >= 0)
        {
            yield return null;
            m_TimeToStart -= Time.deltaTime * k_CountdownSpeed;  // k_CountdownSpeed = 1.5f;
        }
        ...
        // to manage the loading data
        HelperUtil.CallAfterDelay(() => 
        {
            LoadoutState.instance.splashBlur.SetActive(false);
    ...
    public IEnumerator Begin()  
    {
        if (!m_Rerun)
        {
           ...
           // Camera.main.transform.SetParent(characterCollider.transform, true);
            CameraFollow.instance.Target = characterCollider.transform;
    ...
    void Update()
    {
        ...
        if (!m_IsTutorial)
        {
            if (!IsBoostActive)
            {
                if (m_Speed < maxSpeed || prevSpeed < maxSpeed)
                {
                    if (m_Speed != prevSpeed) m_Speed = prevSpeed;
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    public IEnumerator SpawnNewSegment()
    {
        ...
        else
        {
            ...
            currentExitRotation = transform.rotation; // <---------pppppp
        }

        //newSegment.transform.rotation = currentExitRotation; // <---------pppppp
    ...
    public void SpawnObstacle(TrackSegment segment)
    {
        if (segment.possibleObstacles.Length != 0)
        {
            for (int i = 0; i < segment.obstaclePositions.Length; ++i)
            {
                ...
                //AssetReference assetRef = segment.possibleObstacles[16];
