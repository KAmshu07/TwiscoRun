...
public class CharacterCollider : MonoBehaviour
{
	...
	[SerializeField] private bool vitaboostbool = false;
	[SerializeField] private bool vitaspeedbool = false;
    ...
    protected void OnTriggerEnter(Collider c)
    {
        ...
        else if(c.gameObject.layer == k_ObstacleLayerIndex)
        {
            ...
            if (m_Invincible || controller.IsCheatInvincible())
			{
				...
				Debug.Log(vitaspeedbool + " " + vitaboostbool);
                if (vitaboostbool && vitaspeedbool)
                    c.gameObject.GetComponent<BinCollisionCheck>().X();
                if (!vitaspeedbool)
					c.gameObject.GetComponent<BinCollisionCheck>().X();
        ...
        else if(c.gameObject.layer == k_PowerupLayerIndex)
        {
            ...
            if(consumable != null)
            {
				...
				{
					//this is the boolean value that keeps the drum safe from breaking if player collided with ScoreMultiplier(Clone)
					//vitaspeedbool = true;
					StartCoroutine(_On_VitaSpeed());
					Debug.Log("This is now called to Vitaspeed");
					Debug.Log("VITA-----S");
				}
				else
				{
				...
				if(c.gameObject.tag == "Invincibility")
				{
					//this is the boolean value that keeps the drum safe from breaking if player collided with ScoreMultiplier(Clone)
					//vitaboostbool = true;
					StartCoroutine(_On_VitaBoost());
					Debug.Log("This is now called to vitaboost");
				}
				else
				{
					//vitaboostbool = false;
	...
	IEnumerator _On_VitaSpeed()
	{
		vitaspeedbool = true;
		yield return new WaitForSeconds(5f);
		vitaspeedbool = false;
	}
	IEnumerator _On_VitaBoost()
	{
		vitaboostbool = true;
